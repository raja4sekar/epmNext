sap.ui.jsview("launchpad.main", {
	...
	*/ 
	createContent : function(oController) {
		...
		var uisBlueTile = new sap.m.StandardTile({
			icon : "sap-icon://widgets",
			info : sap.app.i18n.getText("HANA_UIS_BLUE"),
			infoState : "None",
			removable : false,
			press : function(oEvent) {
				view.handlePress(oController, 6);						
			}
		});
		uisBlueTile.addStyleClass('templateTileClass');
		uisBlueTile.addStyleClass('uisbClass');
		
		var uisGoldTile = new sap.m.StandardTile({
			icon : "sap-icon://widgets",
			info : sap.app.i18n.getText("HANA_UIS_GOLD"),
			infoState : "None",
			removable : false,
			press : function(oEvent) {
		    	view.handlePress(oController, 7);						
			}
		});		
		uisGoldTile.addStyleClass('templateTileClass');
		uisGoldTile.addStyleClass('uisgClass');	
		...
		var inatoolkitTile = new sap.m.StandardTile({
			icon : "sap-icon://widgets",
			info : sap.app.i18n.getText("INATOOLKIT"),
			infoState : "None",
			removable : false,
			press : function(oEvent) {
				view.handlePress(oController, 9);					
			}
		});
		inatoolkitTile.addStyleClass('templateTileClass');
		inatoolkitTile.addStyleClass('inatoolkitClass');
		...
		var items = [ adminTile, poTile, soTile, sowTile,
			          userTile, uisBlueTile, uisGoldTile,
			          spatialGoldTile, inatoolkitTile,
			          salesMobileTile, fioriTile
			          ];

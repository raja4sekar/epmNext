...
function my_create_after_exit(param) {
	...
	try {
		...
		pStmt = param.connection
				.prepareStatement('select "sap.hana.democontent.epmNext.data::employeeSeqId".NEXTVAL from dummy');
		...
		pStmt = param.connection
				.prepareStatement('insert into SAP_HANA_EPM_NEXT."sap.hana.democontent.epmNext.data::User.Details" values(?,?,?,?)');
		pStmt.setString(1, persNo);
